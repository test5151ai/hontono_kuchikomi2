...
class PendingUsers {
    ...
    updateTabCounts(submittedCount, rejectedCount, notSubmittedCount, approvedCount) {
        ...
        const notSubmittedTab = document.getElementById('not-submitted-tab');
        const approvedTab = document.getElementById('approved-tab');
        
        // 要素が存在する場合のみ更新
        if (allTab) allTab.textContent = `全ユーザー (${totalCount})`;
        if (pendingTab) pendingTab.textContent = `承認待ち (${submittedCount})`;
        if (rejectedTab) rejectedTab.textContent = `拒否済み (${rejectedCount})`;
        if (notSubmittedTab) notSubmittedTab.textContent = `未提出 (${notSubmittedCount})`;
        if (approvedTab) approvedTab.textContent = `承認済み (${approvedCount})`;
    }
