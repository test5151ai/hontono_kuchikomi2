...
## 1. データベース設計と実装 ✅
- [x] ユーザーテーブルの拡張
  - [x] `documentStatus`フィールドの追加（未提出/提出済/承認済/拒否）
  - [x] `documentSubmittedAt`（提出日時）フィールドの追加
  - [x] `documentVerifiedAt`（確認日時）フィールドの追加
  - [x] `documentVerifiedBy`（確認者ID）フィールドの追加
  - [x] `documentRejectReason`（拒否理由）フィールドの追加
- [x] 書類ファイル管理テーブルの作成（必要に応じて）
...
# 5ちゃんねるライクな掲示板機能拡張タスク

## 1. スレッド投稿制限機能
### 1000レス制限の実装
- [ ] スレッドモデルに `isClosed` または `isLocked` フィールドを追加
- [ ] 投稿数が1000件に達したときにスレッドをロックする機能を実装
- [ ] ロック時のUI表示の実装（「このスレッドは終了しました」メッセージ）
- [ ] 閲覧モードと投稿モードの区別
- [ ] 管理者用のロック解除機能（オプション）

## 2. 次スレ自動作成機能
### スレッド自動連鎖システム
- [ ] スレッドモデルに連鎖関連フィールド（`previousThreadId`、`nextThreadId`）を追加
- [ ] 1000レス到達時の次スレ自動作成ロジックの実装
- [ ] タイトルパターン解析と連番付与機能（「その2」「Part2」など）
- [ ] 元スレの店舗情報・メタデータを新スレにコピーする機能
- [ ] スレッド間の相互リンク生成（「前スレ」「次スレ」）
- [ ] スレッド移行通知の自動投稿

### 過去スレ管理・表示
- [ ] 店舗情報セクションに過去スレリストを表示
- [ ] スレッド系列の表示・ナビゲーション機能
- [ ] 過去スレのアーカイブ表示機能
...
- [ ] 各投稿に明確なレス番号を表示
- [ ] 投稿時に自動的にレス番号を生成する機能
- [ ] レス番号でのジャンプ機能（URLハッシュ対応）

### アンカー・参照機能
- [ ] 投稿内容の `>>番号` パターンを検出して自動リンク化
- [ ] アンカークリック時のスクロール・ハイライト機能
- [ ] 投稿フォームへの引用ボタン追加
- [ ] 複数引用のサポート（>>1-3 などの範囲指定）
- [ ] アンカーホバー時のプレビュー表示

## 4. ページネーション改善
### ページ表示の最適化
- [ ] カスタマイズ可能なページネーション（1ページあたりのレス表示数）
- [ ] 「最新50」「全部読む」「最初から」などの表示オプション
- [ ] スクロール位置の記憶・復元機能
- [ ] ページ間移動時の読み込み最適化
...
- [ ] スマートフォン表示の最適化
- [ ] ページネーション操作の使いやすさ向上
- [ ] タッチジェスチャー対応（スワイプでページ切り替えなど）

## 5. ストレージと最適化
### 大規模データ処理
- [ ] 1000レスを超えるスレッドのアーカイブ処理
- [ ] パフォーマンス最適化（キャッシュ、クエリ効率化）
- [ ] 画像・メディア投稿の最適化
- [ ] データベースインデックス改善

## 優先実装項目（第1フェーズ）
1. レス番号とアンカー機能（基本的な表示と参照機能）
2. 1000レス制限の実装（基本的なスレッドロック機能）
3. ページネーション改善（より使いやすいページ切り替え）
4. スレッド連鎖機能の基礎実装（次スレへの移行機能）

# 5chスタイル掲示板機能の実装タスク

## 1. 投稿番号（レス番）機能
- [x] 投稿テーブルに投稿番号フィールドを追加
  - [x] マイグレーションファイルの作成
  - [x] Postモデルの更新
- [x] 投稿番号の自動割り当て処理の実装
  - [x] スレッド内で連番を割り当てる
  - [x] 最初の投稿は1から始まる
  - [x] 新規投稿時に適切な番号を割り当てる
- [x] 投稿番号を表示するフロントエンド実装
  - [x] スレッド表示画面での投稿番号表示
  - [x] 投稿番号が連番になっていることの確認
- [x] エラー修正と動作確認
  - [x] 番号割り当ての不具合修正
  - [x] 投稿順序が正しく表示されることの確認
